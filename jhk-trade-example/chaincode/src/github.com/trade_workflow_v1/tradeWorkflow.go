...
func (t *TradeWorkflowChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "lenderMakePayment" {
		return t.lenderMakePayment(stub, creatorOrg, creatorCertIssuer, args)
	} else if function == "lenderRequestPayment" {
		return t.lenderRequestPayment(stub, creatorOrg, creatorCertIssuer, args)
	} else if function == "importerMakePayment" {
		return t.importerMakePayment(stub, creatorOrg, creatorCertIssuer, args)
...
// Make a payment
func (t *TradeWorkflowChaincode) lenderMakePayment(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	var shipmentLocationKey, paymentKey, tradeKey, lcKey string
	var paymentAmount, expBal, lenBal int
	var shipmentLocationBytes, paymentBytes, tradeAgreementBytes, lenBalBytes, expBalBytes, lcBytes []byte
	var tradeAgreement *TradeAgreement
	var letterOfCredit *LetterOfCredit
	var err error
	...
	if len(args) != 3 {
		err = errors.New(fmt.Sprintf("Incorrect number of arguments. Expecting 3: {Trade ID, L/C ID, Expiration Date}. Found %d", len(args)))
		return shim.Error(err.Error())
	}
	...
	// lookup Letter of Credit
	lcKey, err = getLCKey(stub, args[0])
	if err != nil { return shim.Error("LC:" + err.Error()) }
	lcBytes, err = stub.GetState(lcKey)
	if err != nil { return shim.Error("LCB:" + err.Error()) }

	// Unmarshal the JSON
	err = json.Unmarshal(lcBytes, &letterOfCredit)
	if err != nil { return shim.Error("ULC:" + err.Error()) }

	// Verify L/C has been accepted
	if letterOfCredit.Status != ACCEPTED {
		fmt.Printf("Letter of Credit for trade %s has not been accepted yet", args[0])
		return shim.Error("L/C not accepted yet")
	}
	...
	if err != nil {
		return shim.Error("BLAHHH1:" + err.Error())
	}
	expBal, err = strconv.Atoi(string(expBalBytes))
	if err != nil {
		return shim.Error("BLA1:" + err.Error())
	}
	lenBalBytes, err = stub.GetState(lenBalKey)
	if err != nil {
		return shim.Error("BLAHHH2:" + err.Error())
	}
	lenBal, err = strconv.Atoi(string(lenBalBytes))
	if err != nil {
		return shim.Error("BLA2:" + err.Error())
	}
	...
	//}
	paymentAmount = tradeAgreement.Amount // - DISCOUNT(tradeAgreement.Amount)
	...
	if lenBal < paymentAmount {
		...
		return shim.Error("insufficient lender balance")
	}
	lenBal -= paymentAmount

	// create acceptedLC to be paid back by importer's bank
	acceptedLC := &AcceptedLC{args[1], args[2], paymentAmount, 0, REQUESTED}
	acceptedLCBytes, err := json.Marshal(acceptedLC)
	if err != nil {
		return shim.Error("Error marshaling acceptedLC structure")
	}

	// Write the state to the ledger
	acceptedLCKey, err := getAcceptedLCKey(stub, args[1])
	if err != nil {
		return shim.Error(err.Error())
	}
	err = stub.PutState(acceptedLCKey, acceptedLCBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
	fmt.Printf("Accepted LC payment to exporter for trade %s recorded\n", args[0])
...
// Lender request a payment
func (t *TradeWorkflowChaincode) lenderRequestPayment(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	var acceptedLCKey string
	var acceptedLCBytes []byte
	var acceptedLC *AcceptedLC
	var err error
	...
	if len(args) != 1 {
		err = errors.New(fmt.Sprintf("Incorrect number of arguments. Expecting 1: {L/C ID}. Found %d", len(args)))
		return shim.Error(err.Error())
	}

	// lookup Accepted LC
	acceptedLCKey, err = getAcceptedLCKey(stub, args[0])
	if err != nil { return shim.Error(err.Error()) }
	acceptedLCBytes, err = stub.GetState(acceptedLCKey)
	if err != nil { return shim.Error(err.Error()) }

	if len(acceptedLCBytes) == 0 {
		err = errors.New(fmt.Sprintf("No record found for LC ID %s", args[0]))
		return shim.Error(err.Error())
	}

	// Unmarshal the JSON
	err = json.Unmarshal(acceptedLCBytes, &acceptedLC)
	if err != nil { return shim.Error(err.Error()) }

	if acceptedLC.Status != REQUESTED {
		fmt.Printf("cannot issue lender to importer payment request due to incorrect acceptedLC state: %s %s\n", args[0], acceptedLC.Status)
		return shim.Error("Incorrect AcceptedLC Status")
	} else {
		acceptedLC.Status = ISSUED
	}
	acceptedLCBytes, err = json.Marshal(acceptedLC)
	if err != nil {
		return shim.Error("Error marshaling AcceptedLC structure")
	...
	fmt.Printf("Issued payment to importer for LC %s recorded\n", args[0])

	return shim.Success(nil)
}

// Make a payment
func (t *TradeWorkflowChaincode) importerMakePayment(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	var paymentAmount, impBal int
	var impBalBytes []byte
	var acceptedLC *AcceptedLC
	var err error
	...
	if len(args) != 1 {
		err = errors.New(fmt.Sprintf("Incorrect number of arguments. Expecting 1: {LC ID}. Found %d", len(args)))
		return shim.Error(err.Error())
	}

	// Check if there's already a lender payment request
	acceptedLCKey, err := getAcceptedLCKey(stub, args[0])
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	if len(acceptedLCBytes) == 0 {
		fmt.Printf("No payment request found for LC %s", args[0])
		return shim.Error("No payment request found")
	}
	...
	paymentAmount = acceptedLC.Amount
	acceptedLC.Payment += paymentAmount
	lenBal += paymentAmount
