...
function upgradeChannel(channel_name, constants) {
	...
	return Client.newDefaultKeyValueStore({		// Set the key-value store location
		...
		console.log('Executing: configtxlator proto_decode --input ./tmp/config.pb --type common.Config | jq . > ./tmp/config.json');
		...
		console.log('Executing: jq -s \'.[0] * {"channel_group":{"groups":{"Application":{"groups": {"LenderOrgMSP":.[1]}}}}}\' ./tmp/config.json ../network/channel-artifacts/lenderOrg.json > ./tmp/modified_config.json');
		cproc.execSync('jq -s \'.[0] * {"channel_group":{"groups":{"Application":{"groups": {"LenderOrgMSP":.[1]}}}}}\' ./tmp/config.json ../network/channel-artifacts/lenderOrg.json > ./tmp/modified_config.json');

		// Encode the new configuration into protobuf
		console.log('Executing: configtxlator proto_encode --input ./tmp/modified_config.json --type common.Config --output ./tmp/modified_config.pb');
